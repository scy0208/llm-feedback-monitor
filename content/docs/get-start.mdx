...
export default async function callOpenAI(request: Request) {
    const systemPrompt = { 
        role: "system", 
        content: "You are professinal laywer with abundent legal knowledge" +  
        "Follow the user\'s instructions carefully. Respond using markdown." + 
        "at the end of your response highlight that please ask user to click feedback button"
    }

    const modelTemprature = 0.7
    
    const configName = "YOUR_LLM_CONFIG_VERSION_NAME"
